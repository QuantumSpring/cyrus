---
name: bug-reporting-pm
description: Product management workflow for bug reporting in Linear. ALWAYS use this skill when the user mentions bugs, defects, errors, or issues with software behavior. Trigger phrases include "report a bug", "found a bug", "there's a bug", "bug in", "create a bug issue", "file a bug", "log a bug", "something is broken", "not working correctly", "unexpected behavior", "error in", or any mention of defects, software problems, or incorrect functionality. Also use when the user wants to document issues in Linear or create bug tickets. This skill handles the complete workflow from bug information gathering through Linear issue creation and optional implementation triggering.
---
...
```
@claude start implementing this bug fix
```
