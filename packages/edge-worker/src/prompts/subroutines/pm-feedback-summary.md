---
name: feedback-processing-pm
description: Streamlined product management workflow for processing simple requests and feedback in Linear. ALWAYS use this skill when the user mentions feedback, simple requests, user input, or customer suggestions. Trigger phrases include "process feedback", "capture feedback", "user feedback", "customer request", "simple request", "feedback about", or any mention of quick input that needs to be logged. This skill handles lightweight feedback capture with minimal friction - only requiring project selection, then analyzing any user input into a structured format for Linear.
---
